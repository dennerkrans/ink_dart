...
- **Types.** `IntValue` and `FloatValue` stay distinct; follow ink's numeric promotion rules, not Dart's. Every value object implements `==` and `hashCode`.
- **Errors are data.** Collect into `state.currentErrors`, surface via `onError`; throw only when no handler is set.
- **Determinism.** Port the reference PRNG exactly. Every conformance case runs with a fixed seed.
...
1. `npm install inkjs` in `tool/`; use its compiler (`inkjs-compiler`) so no .NET is needed.
2. Vendor inkjs's `.ink` test stories into `test/conformance/cases/`, grouped by phase.
3. Write `tool/regen_goldens.mjs`: compile each `.ink` to JSON, run it in inkjs with a fixed seed and a fixed choice script, emit `<case>.golden.json` containing every line, its tags, every choice list, and the final `state.toJson()`.
